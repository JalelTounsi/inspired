...
async function getPrice() {
  const amountToSell = new BigNumber(document?.getElementById('amountTokenToSell')?.value * 10 ** 18).toFixed()
  ...
  document.getElementById('amountTokenToSell').value = `${swapPriceJSON.sellAmount / 10 ** 18}`
  ...
  document.getElementById('Price').value = `1 ${document.getElementById('TokenToSell').value} = ${priceFixed} ${
    document.getElementById('TokenToBuy').value
  }`
  document.getElementById('garenteedPrice').value = Number.parseFloat(swapPriceJSON.guaranteedPrice).toFixed(5)
  if (swapPriceJSON.orders !== null)
    document.getElementById('liquidityProvider').value = swapPriceJSON.orders[0].source !== null ? swapPriceJSON.orders[0].source : 'no liquidity'
  document.getElementById('estimatedPriceImpact').value = `${Number.parseFloat(swapPriceJSON.estimatedPriceImpact).toFixed(4)} %`
...
export default function SwapTokens() {
  ...
  return (
    <Flex>
      <Box borderRadius="md" p={{ base: 5, lg: 16 }} bgSize={'md'}>
        <Stack spacing={{ base: 4, md: 8, lg: 20 }} direction={{ base: 'column', md: 'row' }}>
          <Box
            ...
            <VStack>
                ...
                <FormControl isRequired>
                  <SimpleGrid columns={2} spacing={2}>
                    <Box>
                      ...
                      <HStack>
                        <Image alt={'Login Image'} src={tokens[8].logoURI} width={8} height={8} id="SellTokenImage" />
                        <Input type="text" name="SellToken" placeholder={tokens[8].symbol} id="TokenToSell" variant={'flushed'} htmlSize={4} />
                      </HStack>
                    </Box>
                    <Box>
                      {/* <FormLabel>Amount To Sell</FormLabel> */}
                      <Input type="text" name="SellAmount" placeholder="0" id="amountTokenToSell" variant={'flushed'} />
                    </Box>
                  </SimpleGrid>
                </FormControl>
              </VStack>
                ...
                <FormControl>
                  <SimpleGrid columns={2} spacing={2}>
                    <Box>
                      ...
                      <HStack>
                        <Image alt={'Login Image'} src={tokens[1].logoURI} width={8} height={8} id="BuyTokenImage" />
                        <Input type="text" name="BuyToken" placeholder={tokens[1].symbol} id="TokenToBuy" variant={'flushed'} htmlSize={4} />
                      </HStack>
                    </Box>
                    <Box>
                      {/* <FormLabel>Amount To Buy</FormLabel> */}
                      <Input type="text" name="BuyAmount" placeholder="amount To Buy" id="amountTokenToBuy" readOnly={true} variant={'flushed'} />
                    </Box>
                  </SimpleGrid>
                </FormControl>
              </VStack>
              ...
              {/* the intel about the transaction */}
              <SimpleGrid columns={2} spacing={2}>
                <Box maxW="sm">
                  <FormControl>
              ...
              <SimpleGrid columns={2} spacing={2}> */}
                <Box>
                  ...
                  <Input type="text" name="Price" placeholder="Price" id="Price" readOnly={true} display="none" border={'none'} variant={'flushed'} />
                </Box>
                <Box>
